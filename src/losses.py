...
class CauchyLoss(nn.Module):
    ...
    def forward(self, pred, target):
        if pred.ndim != target.ndim:
            if pred.ndim > target.ndim and pred.shape[-1] == 1:
                pred = pred.squeeze(-1)
...
class PhysicsInformedLoss(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        s_min = speed_min if speed_min is not None else getattr(Config, 'SPEED_MIN', 10.0)
        s_max = speed_max if speed_max is not None else getattr(Config, 'SPEED_MAX', 140.0)
        self.bound = DomainBoundaryLoss(speed_min=s_min, speed_max=s_max)
        
        pw = physics_loss_weight if physics_loss_weight is not None else physics_weight
        self.physics_weight = float(pw)
    ...
    def forward(self, pred, target, speed_seq=None):
        ...
        # add kinematic acceleration penalty if trajectory provided or if pred is a sequence
        if speed_seq is not None and speed_seq.shape[-1] > 1:
            total_loss = total_loss + self.physics_weight * self.physics(speed_seq)
        elif pred.ndim >= 2 and pred.shape[-1] > 1:
            total_loss = total_loss + self.physics_weight * self.physics(pred)
