...
squeeze_excite_block = SEBlock1D
...
class Factorized1DNet(nn.Module):
    def __init__(self, in_channels=1, base_filters=64, se_reduction=8, dropout=0.2, kernel_size=7):
        ...
        sinc_kernel = getattr(Config, 'SINC_KERNEL_SIZE', 251)
        sinc_stride = getattr(Config, 'SINC_STRIDE', 16)
        self.frontend = SincConv1d(out_channels=base_filters, kernel_size=sinc_kernel, stride=sinc_stride)
    ...
    def forward(self, x):
        # Gracefully handle 4D inputs (e.g. (B, 1, 128, 313) or (B, C, H, W))
        # and 3D spectrogram inputs (e.g. (B, 128, 313) or (1, 128, 313))
        if x.ndim == 4:
...
def build_model(config=Config, **kwargs):
    base_f = kwargs.get('base_filters', getattr(config, 'BASE_FILTERS', 64))
    se_red = kwargs.get('se_reduction', getattr(config, 'SE_REDUCTION', getattr(config, 'SE_RATIO', 8)))
    drop = kwargs.get('dropout', getattr(config, 'DROPOUT', getattr(config, 'DROPOUT_RATE', 0.2)))
    kernel_time = kwargs.get('kernel_size', getattr(config, 'KERNEL_SIZE_TIME', 7))
    in_channels = kwargs.get('in_channels', getattr(config, 'IN_CHANNELS', 1))
...
def build_se_resnet(input_shape=None, config=Config, dropout=None, se_ratio=None, **kwargs):
    base_f = kwargs.get('base_filters', getattr(config, 'BASE_FILTERS', 64))
    se_red = se_ratio if se_ratio is not None else kwargs.get('se_reduction', getattr(config, 'SE_REDUCTION', getattr(config, 'SE_RATIO', 8)))
    drop = dropout if dropout is not None else kwargs.get('dropout', getattr(config, 'DROPOUT', getattr(config, 'DROPOUT_RATE', 0.2)))
    kernel_time = kwargs.get('kernel_size', getattr(config, 'KERNEL_SIZE_TIME', 7))
    in_channels = kwargs.get('in_channels', getattr(config, 'IN_CHANNELS', 1))
    return Factorized1DNet(
        in_channels=in_channels,
