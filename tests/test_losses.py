...
# --- Tests for src.losses implementations ---

from src.losses import (
    PhysicsInformedLoss,
    CauchyLoss as SrcCauchyLoss,
    HuberLoss as SrcHuberLoss,
    DomainBoundaryLoss,
    KinematicAccelerationLoss,
)


def test_src_losses_physics_informed_loss_scalar_and_broadcast_safe():
    """Verifies that PhysicsInformedLoss safely handles (B, 1) and (B,) without broadcasting bugs."""
    criterion = PhysicsInformedLoss(gamma=5.0, physics_weight=0.1, bound_weight=0.05)
    
    # Matching (B, 1) and (B,)
    pred_2d = torch.tensor([[60.0], [70.0]], requires_grad=True)
    target_1d = torch.tensor([62.0, 68.0])
    
    loss = criterion(pred_2d, target_1d)
    assert loss.ndim == 0, f"Expected scalar loss, got shape {loss.shape}"
    loss.backward()
    assert pred_2d.grad is not None
    assert not torch.isnan(pred_2d.grad).any()


def test_src_losses_domain_boundary_penalty():
    """Verifies DomainBoundaryLoss enforces [10.0, 140.0] km/h bounds."""
    bound_loss = DomainBoundaryLoss(speed_min=10.0, speed_max=140.0)
    
    # Valid speeds: zero penalty
    valid_speeds = torch.tensor([15.0, 50.0, 100.0, 135.0])
    assert bound_loss(valid_speeds).item() == 0.0
    
    # Speeds below min and above max
    out_of_bounds = torch.tensor([5.0, 150.0])
    penalty = bound_loss(out_of_bounds)
    expected = (10.0 - 5.0)**2 + (150.0 - 140.0)**2  # 25 + 100 = 125, mean = 62.5
    assert np.isclose(penalty.item(), expected / 2.0, atol=1e-5)


def test_src_losses_kinematic_acceleration():
    """Verifies KinematicAccelerationLoss on sequential trajectories."""
    kin_loss = KinematicAccelerationLoss(max_accel=30.0, dt=1.0)
    
    # Scalar: returns 0.0
    scalar = torch.tensor([[60.0]])
    assert kin_loss(scalar).item() == 0.0
    
    # Trajectory within limits: returns 0.0
    traj_valid = torch.tensor([[50.0, 65.0, 80.0]])
    assert kin_loss(traj_valid).item() == 0.0
    
    # Trajectory exceeding limits: returns positive penalty
    traj_invalid = torch.tensor([[50.0, 100.0]])  # diff = 50 > 30 => excess = 20 => penalty = 400
    assert np.isclose(kin_loss(traj_invalid).item(), 400.0, atol=1e-5)
